...
<mssqlcrt:provider type="prefix" managed="yes" native="yes">Microsoft.Mashup.OleDb provider.0.1</mssqlcrt:provider>
<mssqlcrt:provider type="prefix" managed="yes" native="yes">Microsoft Mashup Engine.2</mssqlcrt:provider>
...
<xsl:otherwise>
<xsl:choose>
<xsl:when test="name(..) != 'Statement' and name(..) != 'Insert' and name(..) != 'Union' and name(..) != 'UnionAll' and name(..) != 'OpSynapseCorrelationId' and name(..) != 'AddQueryHint' and name(..) != 'concatenate-children'">
(
</xsl:when>
<xsl:when test="name(..) = 'Statement' and count(.//Recursion) != 0">
; WITH
...
<xsl:if test="name(..) != 'Statement' and name(..) != 'Insert' and name(..) != 'Union' and name(..) != 'UnionAll' and name(..) != 'OpSynapseCorrelationId' and name(..) != 'AddQueryHint' and name(..) != 'concatenate-children'">
)
</xsl:if>
</xsl:otherwise>
</xsl:choose>
...
<xsl:choose>
<xsl:when test="count(../../../Sources//OpHash)=0">
APPROX_COUNT_DISTINCT(CAST(ISNULL(<xsl:apply-templates select="./*[1]"/>, 0x) AS VARBINARY))
</xsl:when>
<xsl:otherwise>
<xsl:apply-templates select="./*[1]"/>
</xsl:otherwise>
...
<!-- Remote data source expression -->
<!-- TODO: Value.NativeQuery is probably not going to work in general (should instruct M to generate WITH MEMBER at the beginning of the query instead of inline somehow)
Also need to figure out how to input arbitrary expression (e.g. check if expression with <> works or not) -->
<xsl:template match="OpRemoteExpression">
Value.NativeQuery(<xsl:call-template name="quote-string">
<xsl:with-param name="string" select="."/>
</xsl:call-template>)
</xsl:template>

<!-- Remote Measure Reference -->
<xsl:template match="OpRemoteMeasure">
[Cube.ApplyMeasure](<xsl:call-template name="quote-string"><xsl:with-param name="string" select="."/></xsl:call-template>)
</xsl:template>

<!-- Remote Dimension Reference -->
<xsl:template match="OpRemoteDimension">
[Cube.Dimension](<xsl:call-template name="quote-string"><xsl:with-param name="string" select="./CubeName"/></xsl:call-template>,<xsl:call-template name="quote-string"><xsl:with-param name="string" select="./TableName"/></xsl:call-template>)
</xsl:template>

<!-- Cube M query lifted as function case -->
<xsl:template match="OpCubeName">
<xsl:choose>
<xsl:when test="@IsFunction = 'true'">#"<xsl:value-of select="."/>"([])</xsl:when>
<xsl:otherwise>#"<xsl:value-of select="."/>"</xsl:otherwise>
</xsl:choose>
</xsl:template>

<!-- M-Query to discover DQ Capabilities -->
<xsl:template match="OpDQCapabilityDiscovery">
let
getCaps = (getCube, getTable) => let
table = Table.FromValue(try getCube() otherwise getTable()),
flat = Table.RemoveColumns(table, Table.ColumnsOfType(table, {type table, type record, type list})),
cubeCaps = #table({"Name", "Value"}, if Value.Metadata(flat)[Cube.Cube]? &lt;&gt; null then {{"Cube", null}} else {}),
dqCaps = DirectQueryCapabilities.From(flat)
in
cubeCaps &amp; dqCaps
in
getCaps(() => <xsl:apply-templates select="./OpCubeName"/>, () => #"<xsl:value-of select="./TableName"/>")
</xsl:template>

<!-- M-Query to discover DQ Capabilities for Expressions -->
<xsl:template match="OpDQExpressionCapabilityDiscovery">
let
table = Table.FromValue(<xsl:value-of select="./Expression"/>),
flat = Table.RemoveColumns(table, Table.ColumnsOfType(table, {type table, type record, type list})),
dqCaps = DirectQueryCapabilities.From(flat)
in
dqCaps
...
<xsl:template match="OpIsNotNull">
...
APPROX_COUNT_DISTINCT(CAST(ISNULL(<xsl:call-template name="print-children-list">
<xsl:with-param name="operator" select="', 0x) AS VARBINARY) + CAST(ISNULL('"/>
</xsl:call-template>, 0x) AS VARBINARY))
</xsl:template>

<!-- No-op for msql.-->
<xsl:template match="OpHash64">
<xsl:apply-templates select="./OpHash[1]"/>
</xsl:template>
...
<!-- M parameter values to MSQL         -->

<!-- Cube type models, hardcoded value CubeCTE as per contract with M Engine         -->
<xsl:template match="CubeRecordRef">
<xsl:choose>
<xsl:when test="@IsParams = 'true'"><xsl:text>,</xsl:text></xsl:when>
<xsl:otherwise><xsl:text>with </xsl:text></xsl:otherwise>
</xsl:choose>
<xsl:text>CubeCTE as </xsl:text>(select * from <xsl:apply-templates select="./*[1]"/>(recordref(<xsl:call-template name="nonemptyquote-string">
<xsl:with-param name="string" select="./ParameterRecordName"/>
</xsl:call-template>
<xsl:text>)))</xsl:text>
</xsl:template>

<!-- (recordref('ParameterRecord'))-->
<xsl:template match="RecordRef">
<xsl:apply-templates select="./*[1]"/>(recordref(<xsl:call-template name="nonemptyquote-string">
<xsl:with-param name="string" select="./ParameterRecordName"/>
</xsl:call-template>
<xsl:text>))</xsl:text>
</xsl:template>

<!-- with listparams ... ParameterRecord as( SELECT listref('StartTimeParameter') as StartTimeParameter,CAST( '2016-01-09 00:00:00' AS datetime) AS EndTimeParameter) -->
<xsl:template match="MParameterRecords">
<xsl:text>with </xsl:text>
<xsl:apply-templates select="./ListParameters"/><xsl:text> ParameterRecord as </xsl:text>
<xsl:apply-templates select="./Select"/>
</xsl:template>

<xsl:template match="ListParameters">
<xsl:apply-templates select="./ListParam"/>
</xsl:template>

<!-- StartTimeParameter as (select CAST( '2016-01-02 00:00:00' AS datetime) AS 'col' union all select CAST( '2016-02-02 00:00:00' AS datetime) AS 'col'), -->
<xsl:template match="ListParam">
<xsl:value-of select="./@Name"/><xsl:text> as (select </xsl:text>
<xsl:call-template name="print-children-list">
<xsl:with-param name="operator" select="' union all select '"/>
</xsl:call-template>
<xsl:text>), </xsl:text>
</xsl:template>

<!-- listref('RepoNameParameter') as RepoNameParameter -->
<xsl:template match="ListRefParams">
<xsl:param name="parameterName" select="."/>
<xsl:text>listref('</xsl:text><xsl:value-of select="$parameterName"/><xsl:text>') as </xsl:text><xsl:value-of select="$parameterName"/>
</xsl:template>

<!-- Convert a string to the string quotation form only if input is not empty                     -->
<xsl:template name="nonemptyquote-string">
...
<xsl:template match="concatenate-children">
<xsl:call-template name="print-children-list">
