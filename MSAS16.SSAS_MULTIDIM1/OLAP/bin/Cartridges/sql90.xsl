...
<xsl:otherwise>
<xsl:choose>
<xsl:when test="name(..) != 'Statement' and name(..) != 'Insert' and name(..) != 'Union' and name(..) != 'UnionAll' and name(..) != 'OpSynapseCorrelationId' and name(..) != 'AddQueryHint'">
(
</xsl:when>
<xsl:when test="name(..) = 'Statement' and count(.//Recursion) != 0">
; WITH
...
<xsl:if test="name(..) != 'Statement' and name(..) != 'Insert' and name(..) != 'Union' and name(..) != 'UnionAll' and name(..) != 'OpSynapseCorrelationId' and name(..) != 'AddQueryHint'">
)
</xsl:if>
</xsl:otherwise>
</xsl:choose>
<xsl:apply-templates select="./SynapseOption"/>
</xsl:template>
...
<xsl:choose>
<xsl:when test="name()='OpLOG'">(LOG(<xsl:value-of select="$Op2"/>) / LOG(<xsl:value-of select="$Op1"/>))</xsl:when>
</xsl:choose>
</xsl:template>
<!--
...
<xsl:choose>
<xsl:when test="@Op1Nullable = 'false' or @Op2Nullable = 'false'">
(SELECT DATEADD(mm, AuxTable.Op2, AuxTable.Op1) FROM (SELECT DATEDIFF(dd, 0, CAST(<xsl:value-of select="$Op1"/> AS DATE)) AS Op1, <xsl:value-of select="$Op2"/> AS Op2) AS AuxTable)
</xsl:when>
<xsl:otherwise>
(SELECT COALESCE(DATEADD(mm, AuxTable.Op2, AuxTable.Op1), DATEADD(mm, AuxTable.Op2, CAST(-2 AS datetime)), DATEADD(mm, 0, AuxTable.Op1))
FROM (SELECT DATEDIFF(dd, 0, CAST(<xsl:apply-templates select="./*[1]"/> AS DATE)) AS Op1, <xsl:apply-templates select="./*[2]"/> + 0 AS Op2) AS AuxTable)
</xsl:otherwise>
</xsl:choose>
</xsl:when>
</xsl:choose>
